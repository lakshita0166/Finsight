...
export default function Transactions() {
  ...
  }), [activeTxns, typeFilter, search])

  const dynamicSummary = useMemo(() => {
    const income   = activeTxns.filter(t => ['CREDIT','INTEREST','OPENING'].includes(t.txn_type)).reduce((s,t)=>s+(Number(t.amount)||0),0)
    const expenses = activeTxns.filter(t => ['DEBIT','TDS'].includes(t.txn_type)).reduce((s,t)=>s+(Number(t.amount)||0),0)
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Transactions',  value: activeTxns.length,
              sub: `${filtered.length} shown`,
              color: 'text-slate-700', bg: 'bg-slate-50', border: 'border-slate-100' },
            { label: 'Total Income',  value: fmt(dynamicSummary.total_income),
              sub: `${activeTxns.filter(t=>['CREDIT','INTEREST'].includes(t.txn_type)).length} credits`,
              color: 'text-green-600', bg: 'bg-green-50', border: 'border-green-100' },
            { label: 'Total Expenses',value: fmt(dynamicSummary.total_expenses),
              sub: `${activeTxns.filter(t=>['DEBIT','TDS'].includes(t.txn_type)).length} debits`,
        ...
        {activeTab === 'analytics' && (
          <CategoryBreakdown transactions={activeTxns} />
        )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              ...
              <div className="relative">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)}
                  placeholder="Narration, mode, category..."
                  className="pl-8 pr-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 w-48"/>
              </div>
            </div>
